...
char *convert(char *infix){ //하나의 문자열(공백포함) 들어옴
   operand_stack = create();
   char *postfix = (char *)malloc(strlen(infix)+1); //중위 표기식을 후위표기식으로 변환하기 위한 문자열 할당
   char *pos = postfix; //postfix의 어느 위치에 넣어햐 할지 위치를 표시해주는 pos
